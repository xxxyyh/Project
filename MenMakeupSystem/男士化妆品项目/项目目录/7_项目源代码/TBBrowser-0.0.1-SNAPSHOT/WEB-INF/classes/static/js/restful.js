...
//		console.log(aa);
		if($oinput.attr("type")=="checkbox"){
			...
			var count =  obj.find("[name="+name+"]").find("option").length;
//			console.log(name+"长度==》"+count);
			for(var i=0;i<count;i++)
	         {           
	         	if(obj.find("[name="+name+"]").get(0).options[i].text == ival)
	            {
	                $("#"+name).get(0).options[i].selected = true;
	                break;
	            }else{
	            	$("#"+name).get(0).options[i].selected = false;
	            }
	        }
		}else{
